        ...
        if len(msg) > 0:
            ...
            if msg["action"] == "connect":
                ...
                # connect to the peer
                    ...
                    the_guys = self.group.list_me(from_name)
                    msg = json.dumps(
                        {"action": "play", "status": "success"})
                    for g in the_guys[1:]:
                        to_sock = self.logged_name2sock[g]
                        mysend(to_sock, json.dumps(
                            {"action": "play", "status": "request", "from": from_name}))
                        viper.main()
                else:
                    msg = json.dumps(
                        {"action": "play", "status": "no-user"})
                mysend(from_sock, msg)
...
# ==============================================================================
                ...
                from_name = self.logged_sock2name[from_sock]
                msg = self.group.list_scores(from_name)
                mysend(from_sock, json.dumps(
                    {"action": "list", "results": msg}))
                ...
                from_name = self.logged_sock2name[from_sock]
                n = msg["message"]
                self.group.update_scores(from_name, n)
